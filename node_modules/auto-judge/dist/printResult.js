...
const printResult = (id, sourceFile, totalResult) => {
    if (totalResult.isAllSuccess)
        console.log(chalk.bold.black.bgGreen(' PASS ') + ` ${sourceFile}`);
    else
        console.log(chalk.bold.black.bgRed(' FAIL ') + ` ${sourceFile}`);
    ...
    console.log();
    for (const result of totalResult.testResults) {
        if (result.isSuccess || result.received === 'timeout')
            continue;
    ...
    console.log(chalk.bold('Tests: ') +
        ...
        (totalResult.numberOfTest.passed
            ? chalk.bold.green(`${totalResult.numberOfTest.passed} passed`) + ', '
            : '') +
...
const formatStringByNewLineCharacter = (string) => {
